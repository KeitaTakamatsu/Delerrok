...
void makeCloudAppData(uint8_t* dist, uint8_t* src, int index, uint16_t datalength, uint16_t formattype);
...
CATaskData* taskTransit(CATaskData* taskData, SDBAssignedInfo *sdbInfo)
{
    ...
    /* Get Agency from SDB*/
    uint64_t l = toUInt64(txn_data->agencyID, 0);
...
/* Add Account Data to response message */
#ifdef DEBUG_MODE
...
CATaskData *taskSDBEditor(CATaskData* taskData, SDBAssignedInfo* sdbInfo)
{
    ...
    {       
        ...
        case SDB_INDEX_FARE:
        {
            ...
            uint64_t id = makeFlatFareIDFromFare(&fare);
        ...
        case SDB_INDEX_STATION:
        {
            ...
            SDBAssign(data->sdbindex,  toUInt64(station.stationID, 0));
...
CATaskData *taskTransfer(CATaskData* taskData, SDBAssignedInfo* sdbInfo)
{
    ...
    switch(tf->formatType)
    {
        ...
        case 0x0201:
        {
            ...
            addr.sin_port = htons(*((uint16_t*) tf->data));
